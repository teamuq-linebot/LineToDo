...
來源：`line-cua-win/src/linedb.py`、`line-cua-win/src/watch.py`（相對專案根；line-cua-win 已併入本 repo 子目錄。2026-06-26 實讀）。
...
```
line-cua-win/src/                  # 併入本 repo 的子目錄（相對專案根）
...
新增 `line-cua-win/src/watch_json.py`（相對專案根），**純加法**、重用既有模組，輸出 NDJSON（每則訊息一行 JSON）。
...
- venv python 路徑：`${LINE_CUA_WIN_DIR}/.venv/Scripts/python.exe`（`LINE_CUA_WIN_DIR` 未設時預設 `app.getAppPath()/line-cua-win`，再退回 `__dirname` 相對；可另以 `LINE_PYTHON` env 覆寫）
- 腳本路徑：`${LINE_CUA_WIN_DIR}/src/watch_json.py`（同上目錄解析；可另以 `LINE_WATCH_SCRIPT` env 覆寫）
- 這兩個路徑**不再寫死**：由 `LINE_CUA_WIN_DIR` env ＞ `app.getAppPath()/line-cua-win` ＞ `__dirname` 相對依序解析（見 `src/main/config/lineBridge.ts`），仍可在 `settings.json`／env 覆寫。
- `spawn(python, ['watch_json.py','--once','--limit','500'], { cwd: path.join(lineCuaWinDir, 'src'), env: { ...process.env, PYTHONUTF8:'1', PYTHONIOENCODING:'utf-8' } })`（`lineCuaWinDir` = 上述解析結果）
